...
#include <algorithm>
float CommonMath::Distance(const XMFLOAT3& v1, const XMFLOAT3& v2)
{
...
float CommonMath::DistanceSqaured(const XMFLOAT3& v1, const XMFLOAT3& v2)
{
	XMVECTOR vector1 = XMLoadFloat3(&v1);
	XMVECTOR vector2 = XMLoadFloat3(&v2);
	XMVECTOR vectorSub = XMVectorSubtract(vector1, vector2);
...
float CommonMath::DistanceEstimated(const XMFLOAT3& v1, const XMFLOAT3& v2)
{
	XMVECTOR vector1 = XMLoadFloat3(&v1);
	XMVECTOR vector2 = XMLoadFloat3(&v2);
	XMVECTOR vectorSub = XMVectorSubtract(vector1, vector2);
...
XMFLOAT3 CommonMath::getVectorHalfWayPoint(const XMFLOAT3& a, const XMFLOAT3& b)
{
	return XMFLOAT3((a.x + b.x) * 0.5f, (a.y + b.y) * 0.5f, (a.z + b.z) * 0.5f);
}

bool CommonMath::Collision(const XMFLOAT2& hitBox1Pos, const XMFLOAT2& hitBox1Siz, const XMFLOAT2& hitBox2Pos, const XMFLOAT2& hitBox2Siz)
{
...
float CommonMath::Lerp(const float& value1, const float& value2, const float& amount)
{
	return value1 + (value2 - value1) * amount;
}
XMFLOAT2 CommonMath::Lerp(const XMFLOAT2& a, const XMFLOAT2& b, const float& i)
{
	return XMFLOAT2(Lerp(a.x, b.x, i), Lerp(a.y, b.y, i));
}
XMFLOAT3 CommonMath::Lerp(const XMFLOAT3& a, const XMFLOAT3& b, const float& i)
{
	return XMFLOAT3(Lerp(a.x, b.x, i), Lerp(a.y, b.y, i), Lerp(a.z, b.z, i));
}
XMFLOAT4 CommonMath::Lerp(const XMFLOAT4& a, const XMFLOAT4& b, const float& i)
{
	return XMFLOAT4(Lerp(a.x, b.x, i), Lerp(a.y, b.y, i), Lerp(a.z, b.z, i), Lerp(a.w, b.w, i));
}
XMFLOAT3 CommonMath::Max(const XMFLOAT3& a, const XMFLOAT3& b) {
	return XMFLOAT3(std::max(a.x, b.x), std::max(a.y, b.y), std::max(a.z, b.z));
}
XMFLOAT3 CommonMath::Min(const XMFLOAT3& a, const XMFLOAT3& b) {
	return XMFLOAT3(std::min(a.x, b.x), std::min(a.y, b.y), std::min(a.z, b.z));
}
float CommonMath::Clamp(float val, float min, float max)
{
...
XMFLOAT3 CommonMath::getCubicHermiteSplinePos(const XMFLOAT3& startPos, const XMFLOAT3& endPos
...
XMFLOAT3 CommonMath::getQuadraticBezierPos(const XMFLOAT3& a, const XMFLOAT3& b, const XMFLOAT3& c, const float& t) {
...
XMFLOAT3 CommonMath::getQuadraticBezierPos(const XMFLOAT4& a, const XMFLOAT4& b, const XMFLOAT4& c, const float& t) {
