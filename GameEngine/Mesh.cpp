...
void Node::MoreControl(Graphics& gfx, PSMaterialConstantFullmonte& c)
{
	if (meshPtrs.empty())
	{
		return;
	...
	if (auto pcb = meshPtrs.front()->
		QueryBindable<Bind::PixelConstantBuffer<PSMaterialConstantFullmonte>>())
	{
		ImGui::Text("Material");
	...
	void Show(Graphics& gfx,const char* windowName, const Node& root) noexcept
	{
		...
		if (ImGui::Begin(windowName))
		{
			...
			if (pSelectedNode != nullptr)
			{
				...
				pSelectedNode->MoreControl(gfx, mc);
			}
		}
		ImGui::End();
	}
...
std::unique_ptr<Mesh> Model::ParseMesh(Graphics& gfx, const aiMesh& mesh, const aiMaterial* const* pMaterials)
{
	...
	const std::string base = "Models\\gobber\\"s;
	...
	if (mesh.mMaterialIndex >= 0)
	{
		...
		{
			std::shared_ptr<Bind::Texture> tex = Texture::Resolve(gfx, base + texFileName.C_Str(), 1);
		...
		{
			std::shared_ptr<Bind::Texture> tex = Texture::Resolve(gfx, base + texFileName.C_Str(), 2);
			hasAlphaGloss = tex->HasAlpha();
			bindablePtrs.push_back(std::move(tex));
			hasNormalMap = true;
		}
	...
	if (hasDiffuseMap && hasNormalMap && hasSpecularMap)
	{
		...
		auto pvs = VertexShader::Resolve(gfx, "HLSL\\PhongVSNormalMap.cso");
	...
	else if (hasDiffuseMap && hasNormalMap)
	{
		...
		bindablePtrs.push_back(PixelShader::Resolve(gfx, "HLSL\\PhongPSNormalMap.cso"));
	...
	else if (!hasDiffuseMap && !hasNormalMap && !hasSpecularMap)
	{
		...
		auto pvs = VertexShader::Resolve(gfx, "HLSL\\PhongVSNotex.cso");
		auto pvsbc = pvs->GetBytecode();
		bindablePtrs.push_back(std::move(pvs));

		bindablePtrs.push_back(PixelShader::Resolve(gfx, "HLSL\\PhongPSNotex.cso"));
...
//Modelwithouttexture
ModelWithOutTexture::ModelWithOutTexture(Graphics& gfx, const std::string fileName)
	:
	pWindow(std::make_unique<ModelWindow>())
{
	Assimp::Importer imp;
...
void ModelWithOutTexture::Draw(Graphics& gfx)const noxnd
{
	if (auto node = pWindow->GetSelectedNode())
	{
		node->SetAppliedTransform(pWindow->GetTransform());
...
void ModelWithOutTexture::ShowWindow(Graphics gfx, const char* windowName) noexcept
{
	pWindow->Show(gfx,windowName, *pRoot);
}

ModelWithOutTexture::~ModelWithOutTexture() noexcept
{
}

std::unique_ptr<Mesh> ModelWithOutTexture::ParseMesh(Graphics& gfx, const aiMesh& mesh, const aiMaterial* const* pMaterials)
{	
	using namespace std::string_literals;
	namespace dx = DirectX;
	using Dvtx::VertexLayout;
	...
	auto pvs = VertexShader::Resolve(gfx, "HLSL\\PhongVSWithoutTex.cso");
	auto pvsbc = pvs->GetBytecode();
	bindablePtrs.push_back(std::move(pvs));

	bindablePtrs.push_back(InputLayout::Resolve(gfx, vbuf.GetLayout(), pvsbc));
	bindablePtrs.push_back(PixelShader::Resolve(gfx, "HLSL\\PhongPSWithoutTex.cso"));
...
std::unique_ptr<Node> ModelWithOutTexture::ParseNode(int& nextId, const aiNode& node) noexcept
{
	namespace dx = DirectX;
	// node contain transformation
	const auto transform = dx::XMMatrixTranspose(dx::XMLoadFloat4x4(
