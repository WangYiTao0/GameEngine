	...
	unsigned int m , unsigned int n , float gridSize )
{
...

	{
		Technique Grid;
		{
			...
			//only.AddBindable(Texture::Resolve(gfx, "Data\\Images\\OpenArt\\156.jpg"));
			//only.AddBindable(Texture::Resolve(gfx, "Data\\Images\\OpenArt\\156_norm.jpg", 2u));
			only.AddBindable(Texture::Resolve(gfx, "Data\\Images\\sponza_floor_a_diff.png"));
			//only.AddBindable(Texture::Resolve(gfx, "Data\\Images\\sponza_floor_ddn.jpg", 2u));
			//only.AddBindable(Texture::Resolve(gfx, "null", 3u, App::m_DepthRT->GetShaderResourceView()));
			...
			only.AddBindable(PixelShader::Resolve(gfx, "PhongShadow_PS"));
			//only.AddBindable(PixelShader::Resolve(gfx, "WaterWavesPS.cso", "WaterWavesPS.hlsl"));

			only.AddBindable(PixelConstantBuffer<Material>::Resolve(gfx, pmc, 2u));

			only.AddBindable(InputLayout::Resolve(gfx, layout, pvsbc));

			only.AddBindable(std::make_shared<Blender>(gfx, true, 1.0f));
